...
export default function PersonalData(): JSX.Element {
  ...
  const { q5, setValuesByStep } = useContext(CoronaPresetContext);
  const { setProbabilityByKey } = useContext(GlobalDecisionContext);
  ...
  useEffect(() => {
    ...
    if (tasks.length > 0) {
      setProbabilityByKey(tasks[0][0], tasks[0][1], tasks[0][2], tasks[0][3]).then(
        (val: boolean) => val && setTasks((tasks) => tasks.slice(1))
      );
    }
  }, [tasks, setProbabilityByKey]);
  ...
  const handleChangeAge = (
    ...
    if (valueToSet) {
      setValuesByStep({ ...q5, age: valueToSet }, "q5");
      getAndSaveProbabilities({ sex: q5.sex, age: valueToSet });
    }
  };

  const handleChangeSex = (event: React.ChangeEvent<HTMLInputElement>): void => {
    if (event.target.value) {
      setValuesByStep({ ...q5, sex: (event.target as HTMLInputElement).value }, "q5");
      getAndSaveProbabilities({ age: q5.age, sex: event.target.value });
    }
  };
    ...
    <>
      ...
      <Container maxWidth="md" style={{ marginTop: "2rem", fontSize: "1rem", padding: 0 }}>
        <Grid container spacing={2}>
          ...
          <Grid item xs={6}>
            <RadioGroup aria-label="gender" name="gender-selection" value={q5.sex} onChange={handleChangeSex}>
              <FormControlLabel
                value="F"
