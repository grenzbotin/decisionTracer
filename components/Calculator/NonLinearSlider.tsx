...
const createRange = (
  [fromMin, fromMax]: Array<number>,
  [toMin, toMax]: Array<number>
): ((val: number) => number) => {
  const fromRange = fromMax - fromMin;
  const toRange = toMax - toMin;

  return (fromValue: number) =>
    ((fromValue - fromMin) / fromRange) * toRange + toMin;
};
...
const createScale = (
  ...
  const ranges: Array<
    Array<number[] | ((val: number) => number)>
  > = zippedMarks
  ...
  const sliderValToScaled = (sliderVal: number): number => {
    const desiredRanges: Array<
      number[] | ((val: number) => number)
    > = ranges.find(([, x]) => sliderVal >= x[0] && sliderVal <= x[1]);

    return (
      typeof desiredRanges[0] === "function" && desiredRanges[0](sliderVal)
    );
  };
  ...
  const scaledValToSlider = (scaledVal: number): number => {
    const desiredRanges: Array<
      number[] | ((val: number) => number)
    > = ranges.find(([, , , x]) => scaledVal >= x[0] && scaledVal <= x[1]);

    return (
      typeof desiredRanges[2] === "function" && desiredRanges[2](scaledVal)
    );
  };
